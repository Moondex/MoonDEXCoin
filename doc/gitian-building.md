...
*Setup instructions for a Gitian build of MoonDEX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MoonDEX
...
- [Building MoonDEX](#building-moondex-core)
...
Clone the git repositories for MoonDEX and Gitian.
...
in the MoonDEX repository under 'Fetch and build inputs' to install sources which require
...
To build MoonDEX (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the MoonDEX repository.

...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "moondex" is a composite category enabling all MoonDEX-related debug output
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MoonDEXCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MoonDEXCore
    // Mac: ~/Library/Application Support/MoonDEXCore
    // Unix: ~/.moondexcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MoonDEXCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MoonDEXCore";
#else
    // Unix
